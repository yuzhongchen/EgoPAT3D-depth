...
class EgoPAT3DDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        depth = np.zeros((self.maxclip, 224,224))
        ...
        for idx in range(rangenum):
            ...
            depth_temp = Image.open(io.BytesIO(np.array(depth_grp[f"{idx+1+int(finalsource[2])}"]))).resize((224,224))
            color[idx] = np.array(color_temp)
            depth[idx] = np.array(depth_temp)
        ...
        return gt_xyz, color, depth, rangenum, finalsource, hand, handLM