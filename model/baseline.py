...
class Baseline_Hand(nn.Module):
    def __init__(self, cfg):
        super(Baseline_Hand, self).__init__()
        num_LSTM=2
        midfc_channel=1024
        self.hind=midfc_channel
        if 'ResNet' in cfg.MODEL.ARCH.RGB_BACKBONE:
    ...
    def forward(self,img, hand, LEGHTN, cfg):
        ...
        rgb_feature=self.rgb_backbone(img[:,-1,:,:].float())
        res = t.cat((self.x(rgb_feature).unsqueeze(1),self.y(rgb_feature).unsqueeze(1),self.z(rgb_feature).unsqueeze(1)),1)
